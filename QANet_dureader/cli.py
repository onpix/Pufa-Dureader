...
def parse_args():
    ...
    path_settings.add_argument('--train_files', nargs='+',
                               default=['../data/data_train_preprocessed.json.bak'],
                               help='list of files that contain the preprocessed train data')
    path_settings.add_argument('--dev_files', nargs='+',
                               default=['../data/data_dev_preprocessed.json.bak'],
                               help='list of files that contain the preprocessed dev data')
    path_settings.add_argument('--test_files', nargs='+',
                               default=['../data/data_test_preprocessed.json.bak'],
                               help='list of files that contain the preprocessed test data')
    path_settings.add_argument('--save_dir', default='../data/',
                               help='the dir with preprocessed baidu reading comprehension data')
    path_settings.add_argument('--vocab_dir', default='../data/vocab_QAnet/',
                               help='the dir to save vocabulary')
    path_settings.add_argument('--model_dir', default='../data/models_QAnet/',
                               help='the dir to store models')
    path_settings.add_argument('--result_dir', default='../data/results_QAnet/',
                               help='the dir to output the results')
    path_settings.add_argument('--summary_dir', default='../data/summary_QAnet/',
...
def evaluate(args):
    ...
    model = Model(vocab, args)
    model.restore(model_dir=args.model_dir, model_prefix='qanet_'+str(args.epochs))
    logger.info('Evaluating the model on dev set...')
    dev_batches = dataloader.next_batch('dev', args.batch_size,
                                            pad_id=vocab.get_word_id(vocab.pad_token), pad_char_id=vocab.get_char_id(vocab.pad_token), shuffle=False)
